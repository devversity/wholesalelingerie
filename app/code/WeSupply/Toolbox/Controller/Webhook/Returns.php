...
class Returns extends Action
{
    ...
    public function execute()
    {
        ...
        if ($creditMemo['success'] === TRUE) {
            $processedReturns = [$this->params['reference'] => ['message' => $creditMemo['status-message']]];
        ...
        try {
            ...
            if (!$creditMemo) {
                return [
                    ...
                    'reason' => 'order-locked',
                    'status-title' => 'Refund Failed',
                    'status-message' => __('Credit memo cannot be created. Check Magento order (Status %1).', strtoupper($order->getStatus()))
                ];
            }
            ...
            if ($this->creditMemoManagement->refund($creditMemo, (bool)$this->creditMemoData['do_offline'])) {
                ...
                $this->pushSuccessMessage(__('Created Credit Memo %1 in amount of %2 %3',
